...
第8行：该模式允许为每个原子提供额外的标志，以指示在离子弛豫期间是否将允许更改此原子的相应坐标。如果仅缺陷周围的某些壳或表面附近的层应松弛，则此设置很有用。**选择性动力学输入标签是可选的，如果省略了选择性动力学标签，则第八行将在笛卡尔和直接晶格之间进行切换。**
...
从MS建模完成后导出为cif文件：`file-export-.cif`，用`VAESTA`打开该cif文件，选择`file-export data`，`save as type : .vasp`。将这个`.vasp`文件上传到服务器，将其重命名为`POSCAR`，然后打开`vaspkit`，依次进入`3`，`303`。
