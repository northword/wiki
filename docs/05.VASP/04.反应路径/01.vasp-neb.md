...
修改PBS脚本，使其调用VTST编译的VASP进行计算（需要注意VTST编译的时候是使用哪种mpi进行并行运行的，如下高亮行12-13。如果mpi选择有误，会出现只算一个点其他点不动的情况，这个问题直到我尝试自己编译vasp的时候才解决）。（第13，27行）
...
![neb-N_graphene_O-mep-eps](./assets/neb-N_graphene_O-mep-eps.jpg)
